...
################################################################################
#  contour.r
#             contour plots in 2D and 3D
#
################################################################################

#2D Contours
fmcfust.contour.2d <- function(dat, model, grid=50, drawpoints=TRUE, clusters=NULL,
    ...
    if (missing(main)) {
        msg <- bquote(paste("Multivariate CFUST mixture Model (g = ", .(g), ")", sep=""))
    } else msg=main
    ...
    ColPoint <- ColCont <- c("blue", "red", "black", "Brown", "green", colors()[sample(1:length(colors()),g)])  
    if(!is.null(pcol)) ColPoint <- array(pcol,g) 
    if(!is.null(ccol)) ColCont <- array(ccol,g)        
    ...
    if(is.null(component)) {
        dens <- dfmcfust(X, model$mu, model$sigma, model$delta, model$dof, model$pro)
        densMat <- matrix(dens,nx,ny)
        if(lflag) contour(x, y, densMat, nlevels=levels, add=TRUE, drawlabels=FALSE, lty=1, col = ColCont[1],...)
        else {
            levels <- quantile(densMat, probs=levels)
            contour(x, y, densMat, levels=levels, add=TRUE, drawlabels=FALSE, lty=1, col = ColCont[1],...)         
        }
    } else {
...
cfustDA <- function(g, dat, model) {
    if (g == 1) return(rep(1, nrow(dat)))
    probMat <- matrix(0,g,nrow(dat))
    for(i in 1:g) probMat[i,] <- model$pro[i]*dcfust(dat, model$mu[[i]], model$sigma[[i]], model$delta[[i]], model$dof[i])
...
#3D Contours
fmcfust.contour.3d <- function(dat, model, grid=20, drawpoints=TRUE, levels=0.9, 
    clusters=NULL, xlim, ylim, zlim, xlab, ylab, zlab, main, component=NULL, pcol=NULL, ccol=NULL,...) {
    if(length(levels)==1) {
        if(levels<0) stop("'levels' cannot have negative value")
        if(is.whole(levels)) lflag<- T
        else {if(levels>1) stop("'levels' must be either an integer or a quantile value") else lflag<-F}
    ...
    #library(rgl)
    ColPoint <- ColCont <- c("magenta", "green", "red", "blue", "grey", colors()[sample(1:length(colors()),g)])   
    if(!is.null(pcol)) ColPoint <- array(pcol,g)   
    if(!is.null(ccol)) ColCont <- array(ccol,g) 
       
    if(drawpoints) { 
        tdat <- dat; tclusters <- clusters    #cut points that are out of range
        Ind <- which(tdat[,1]>=xlim[1] & tdat[,1]<=xlim[2]); tdat <- tdat[Ind,]; tclusters <- tclusters[Ind]
        Ind <- which(tdat[,2]>=ylim[1] & tdat[,2]<=ylim[2]); tdat <- tdat[Ind,]; tclusters <- tclusters[Ind] 
        Ind <- which(tdat[,3]>=zlim[1] & tdat[,3]<=zlim[2]); tdat <- tdat[Ind,]; tclusters <- tclusters[Ind]                                              
        plot3d(tdat[,1], tdat[,2], tdat[,3], col=ColPoint[tclusters], xlab=xlab, ylab=ylab, zlab=zlab, xlim=xlim, ylim=ylim, zlim=zlim)
#        plot3d(tdat[tclusters==1,1], tdat[tclusters==1,2], tdat[tclusters==1,3], 
#            col=ColPoint[1], xlab=xlab, ylab=ylab, zlab=zlab, xlim=xlim, ylim=ylim, zlim=zlim)
#        if(g>=2){ 
#            for(i in 2:g)            
#            points3d(tdat[tclusters==i,1], tdat[tclusters==i,2], tdat[tclusters==i,3], col=ColPoint[i])        
#        }
    } else {
        plot3d(1,1,1,col=rgb(1,1,1,alpha=1), xlab=xlab, ylab=ylab, zlab=zlab, xlim=xlim, ylim=ylim, zlim=zlim)        
    } 
    XVec <- seq(xlim[1], xlim[2], length=grid)       
    YVec <- seq(ylim[1], ylim[2], length=grid)
    ZVec <- seq(zlim[1], zlim[2], length=grid)
    if(is.null(component)) {
        mst.contour <- function(x, y, z) dfmcfust(cbind(x,y,z), model$mu, 
              model$sigma, model$delta, model$dof, model$pro)
        contour3d(mst.contour, levels, XVec, YVec, ZVec, add=T, color=ColCont[1], fill=T, alpha=0.25, nflag=lflag)
    } else{
        ncomp <- length(as.numeric(component))
        if(!all(component %in% c(1:g))) stop("argument 'component' is incorrectly specified!")
        if(length(levels==1)) levels <- rep(levels,ncomp)
        for(i in 1:ncomp){
          mst.contour <- function(x, y, z) dcfust(cbind(x,y,z), model$mu[[component[i]]], 
              model$sigma[[component[i]]], model$delta[[component[i]]], model$dof[component[i]])
          cat("Plotting component ", component[i] ,"\n")
          contour3d(mst.contour, levels[i], XVec, YVec, ZVec, add=T, color=ColCont[component[i]], fill=T, alpha=0.25, nflag=lflag)
          }
    }  
}
